...
function pendingWithoutGrafaind(): void {
...
describe("BnsConnection", () => {
  ...
  const grafaindTendermintUrl = "ws://localhost:23456";
  const grafaindTendermintHttpUrl = "http://localhost:23456";
  ...
  it("can connect to tendermint via HTTP", async () => {
    pendingWithoutGrafaind();
    const connection = await GrafainConnection.establish(grafaindTendermintHttpUrl);
  ...
  it("can start atomic swap", async () => {
    ...
    const txById = (await connection.searchTx({ tags: [grafainSwapQueryTag(querySwapId)] })).filter(
      isConfirmedTransaction,
    );
    expect(txById.length).toEqual(1);
    expect(txById[0].transactionId).toEqual(transactionId);

    const txBySender = (await connection.searchTx({ tags: [grafainSwapQueryTag(querySwapSender)] })).filter(
      isConfirmedTransaction,
    );
    ...
    const txByHash = (await connection.searchTx({ tags: [grafainSwapQueryTag(querySwapHash)] })).filter(
      isConfirmedTransaction,
    );
  ...
  describe("getFeeQuote", () => {
    ...
    //     kind: "grafain/register_username",
    //     creator: {
    //       chainId: connection.chainId(),
    //       pubkey: {
    //         algo: Algorithm.Ed25519,
