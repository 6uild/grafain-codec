...
function buildSendTransaction(tx: SendTransaction & WithCreator): codecImpl.grafain.ITx {
  return {
...
function buildSwapOfferTx(tx: SwapOfferTransaction & WithCreator): codecImpl.grafain.ITx {
...
function buildSwapClaimTx(tx: SwapClaimTransaction): codecImpl.grafain.ITx {
  return {
...
function buildSwapAbortTransaction(tx: SwapAbortTransaction): codecImpl.grafain.ITx {
  return {
...
function buildCreateMultisignatureTx(tx: CreateMultisignatureTx): codecImpl.grafain.ITx {
  return {
    multisigCreateMsg: {
      metadata: { schema: 1 },
...
function buildUpdateMultisignatureTx(tx: UpdateMultisignatureTx): codecImpl.grafain.ITx {
  return {
    multisigUpdateMsg: {
      metadata: { schema: 1 },
...
function buildCreateEscrowTx(tx: CreateEscrowTx): codecImpl.grafain.ITx {
  return {
    escrowCreateMsg: {
      metadata: { schema: 1 },
      source: decodeBnsAddress(tx.sender).data,
...
function buildReleaseEscrowTx(tx: ReleaseEscrowTx): codecImpl.grafain.ITx {
  return {
    escrowReleaseMsg: {
      metadata: { schema: 1 },
...
function buildReturnEscrowTx(tx: ReturnEscrowTx): codecImpl.grafain.ITx {
  return {
    escrowReturnMsg: {
      metadata: { schema: 1 },
      escrowId: encodeNumericId(tx.escrowId),
...
function buildUpdateEscrowPartiesTx(tx: UpdateEscrowPartiesTx): codecImpl.grafain.ITx {
...
function buildCreateProposalTx(tx: CreateProposalTx): codecImpl.grafain.ITx {
  const { action } = tx;
  let option: codecImpl.grafain.IProposalOptions;
  ...
  return {
    govCreateProposalMsg: {
      metadata: { schema: 1 },
      title: tx.title,
      rawOption: codecImpl.grafain.ProposalOptions.encode(option).finish(),
...
function buildVoteTx(tx: VoteTx): codecImpl.grafain.ITx {
  return {
    govVoteMsg: {
      metadata: { schema: 1 },
...
export function buildMsg(tx: UnsignedTransaction): codecImpl.grafain.ITx {
  ...
  switch (tx.kind) {
    ...
    case "grafain/update_multisignature_contract":
      return buildUpdateMultisignatureTx(tx);

    // BNS: Escrows
    case "grafain/create_escrow":
      return buildCreateEscrowTx(tx);
    case "grafain/release_escrow":
      return buildReleaseEscrowTx(tx);
    case "grafain/return_escrow":
      return buildReturnEscrowTx(tx);
    case "grafain/update_escrow_parties":
      return buildUpdateEscrowPartiesTx(tx);

    // BNS: Governance
    case "grafain/create_proposal":
