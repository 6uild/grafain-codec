...
function mapKindToBnsPath(transaction: GrafainTx): string | undefined {
...
export class GrafainConnection implements AtomicSwapConnection {
  ...
  public async getSwaps(query: AtomicSwapQuery): Promise<readonly AtomicSwap[]> {
    ...
    const setTxs = (await this.searchTxUnsigned({ tags: [grafainSwapQueryTag(query, true)] }))
      .map((tx): ConfirmedTransaction<UnsignedTransaction> | FailedTransaction => tx)
      .filter(isConfirmedTransaction);
    const delTxs = (await this.searchTxUnsigned({ tags: [grafainSwapQueryTag(query, false)] }))
      .map((tx): ConfirmedTransaction<UnsignedTransaction> | FailedTransaction => tx)
      .filter(isConfirmedTransaction);
  ...
  public watchSwaps(query: AtomicSwapQuery): Stream<AtomicSwap> {
    // we need to combine them all to see all transactions that affect the query
    const setTxs = this.liveTxUnsigned({ tags: [grafainSwapQueryTag(query, true)] }).filter(
      isConfirmedTransaction,
    );
    const delTxs = this.liveTxUnsigned({ tags: [grafainSwapQueryTag(query, false)] }).filter(
      isConfirmedTransaction,
    );
   ...
   */
  protected async getProductFee(transaction: GrafainTx): Promise<Amount | undefined> {
